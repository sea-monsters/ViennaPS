...
$source = Join-Path $PSScriptRoot 'fluorocarbon_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'fluorocarbon_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_FLUOROCARBON_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'fluorocarbon_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'fluorocarbon_reference.exe'
$checker = Join-Path $OutputDirectory 'fluorocarbon_cpu_checker.exe'
