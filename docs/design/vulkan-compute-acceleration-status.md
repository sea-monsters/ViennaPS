...
| `P6-LA-BASELINE` | `READY-S` — detailed planning published 2026-08-21 in [p6-p7-execution-plan.md](p6-p7-execution-plan.md); entry card `P6-A0-P6-BASELINE-FREEZE`. Governing hardware fact recorded there: local device lacks shaderFloat64, so every device-FP64 row lands DEVICE-PENDING-HARDWARE with CPU-authoritative stages. | FP64 matrix assembly, SpMV, AXPY/dot/norm, deterministic reduction, BiCGSTAB/Jacobi, OOM admission. | 2D/3D field and matrix CPU/Vulkan differential with convergence/residual history; unlocks oxidation stages. |
