...
using Work = viennaps::SelectiveEpitaxyVelocityWork<float>;
using Rate = viennaps::SelectiveEpitaxyMaterialRate<float>;
...
struct PushConstants {
  std::array<float, 4> normalFactors{};
  float rate111 = 0.5F;
  float rate100 = 1.0F;
...
struct VulkanSelectiveEpitaxyVelocityExecutor::State {
  ...
  bool setup(const std::string_view spirvPath,
    ...
    resetUnlocked();
    if (spirvPath.empty())
      return fail(error, "selective-epitaxy SPIR-V path is empty");
  ...
  [[nodiscard]] bool initialized() const {
    ...
    return liveGeneration(session, sessionGeneration) &&
           ...
           pipeline.get() != VK_NULL_HANDLE && descriptorSet != VK_NULL_HANDLE &&
  ...
  [[nodiscard]] bool invoke(Work &work, std::string &error) {
    ...
    if (!liveGeneration(session, sessionGeneration) ||
      ...
      return fail(error, "selective-epitaxy velocity session is not initialized");
    ...
    if (work.coordinates.size() != count || work.materialIds.size() != count ||
      ...
      return fail(error, "selective-epitaxy velocity spans must have equal lengths");
    if (count > std::numeric_limits<std::uint32_t>::max())
      return fail(error, "selective-epitaxy velocity length exceeds Vulkan ABI");
    const auto &parameters = work.parameters;
    if (parameters.materialRates.empty() ||
        parameters.materialRates.size() > kMaxMaterialRates)
      return fail(error, "selective-epitaxy material table is outside its ABI");
    for (const float value : parameters.normalFactors)
      if (!validFloat(value))
        return fail(error, "normal factors are outside strict FP32");
    if (!validFloat(parameters.rate111) || !validFloat(parameters.rate100) ||
        parameters.rate111 < 0.0F || parameters.rate100 < 0.0F)
      return fail(error, "epitaxy rates are outside the admitted domain");
    ...
    if (!checkedBytes(count, sizeof(std::array<float, 4>), coordBytes) ||
      ...
      return fail(error, "selective-epitaxy buffer size overflows");
    ...
    const std::array<VkDeviceSize, 6U> ranges{
        ...
        static_cast<VkDeviceSize>(normalBytes),
        static_cast<VkDeviceSize>(scalarBytes),
        static_cast<VkDeviceSize>(scalarBytes),
    ...
    if (vkResetCommandBuffer(commandBuffer, 0U) != VK_SUCCESS ||
      ...
      return fail(error, "failed to begin selective-epitaxy command buffer");
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                            ...
                            nullptr);
    PushConstants push{};
    push.normalFactors = {parameters.normalFactors[0], parameters.normalFactors[1],
                          parameters.normalFactors[2], 0.0F};
    push.rate111 = parameters.rate111;
    push.rate100 = parameters.rate100;
    ...
    if (dispatchGroups >
      ...
      return fail(error, "selective-epitaxy dispatch exceeds device limits");
    ...
    if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS)
      return fail(error, "failed to end selective-epitaxy command buffer");
    VkSubmitInfo submit{VK_STRUCTURE_TYPE_SUBMIT_INFO};
    submit.commandBufferCount = 1U;
    submit.pCommandBuffers = &commandBuffer;
    if (vkQueueSubmit(session->device().computeQueue(), 1U, &submit,
                      fence.get()) != VK_SUCCESS)
      return fail(error, "failed to submit selective-epitaxy command buffer");
    if (!fence.wait(10'000'000'000ULL, error))
      return false;
    fence.reset();
    if (!outputBuffer.invalidate(error))
      return fail(error, "failed to invalidate selective-epitaxy output");
    ...
    for (std::size_t index = 0U; index < count; ++index) {
      if (!validFloat(candidate[index]) ||
        ...
        message << "Vulkan selective-epitaxy velocity differs from CPU oracle at "
                << index;
...
VulkanSelectiveEpitaxyVelocityExecutor::VulkanSelectiveEpitaxyVelocityExecutor()
    : state_(std::make_shared<State>()) {}

VulkanSelectiveEpitaxyVelocityExecutor::~VulkanSelectiveEpitaxyVelocityExecutor() = default;

bool VulkanSelectiveEpitaxyVelocityExecutor::initialize(
...
void VulkanSelectiveEpitaxyVelocityExecutor::reset() {
  std::lock_guard lock(state_->mutex);
  state_->resetUnlocked();
}

bool VulkanSelectiveEpitaxyVelocityExecutor::isInitialized() const {
  return state_->initialized();
}

VulkanSelectiveEpitaxyVelocityExecutor::Executor
VulkanSelectiveEpitaxyVelocityExecutor::makeExecutor() const {
  ...
  return [state](Work &work, std::string &error) {
    try {
      ...
      error = "unknown Vulkan selective-epitaxy velocity executor failure";
      work.writtenCount = 0U;
      work.complete = false;
      return false;
    }
