...
$source = Join-Path $PSScriptRoot 'strict_row01_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'strict_row01_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_STRICT_ROW_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'strict_row01_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'strict_row01_reference.exe'
$checker = Join-Path $OutputDirectory 'strict_row01_cpu_checker.exe'
