...
Wave 2 续（2026-08-20，S1）：`P5-S1-SURFACE-INTEGRATION-GREEN` 在验证互斥锁下完成，只补 S0 RED 边界要求的三项适配器，CPU 公式/顺序零改动。（1）ray frontier 准入：`gpu/vulkan/ray/vulkan_ray_flux_engine.cpp` 承认 `NeutralTransport<float,2>`（单粒子、无自定义 source、`maxReflections <= 2`）进入有界 CPU-decision frontier 路由，element materialId 与 coverage globalData 镜像 CPU oracle 的 `PointToElementDataSingle`/`PointToElementData` 映射，RNG 消费顺序与 frontier 调度顺序不变。（2）rebuild 边界谓词：主线 CPU 探针实证 S0 几何在 reflective maxIndex 平面合法存有 3 个定义点，`psHrleSparseReconstruction.hpp` 的候选校验由 maxIndex 排他的 `isInDomain` 改为 hrle 原生语义的 `isOutsideOfDomain`；`tests/hrleSparseReconstruction` 新增边界语义回归（reflective maxIndex 通过、periodic maxIndex 拒绝）。sub-agent 建议的"LEVEL_SET 降 CPU 绕过"被审稿否决——实证表明那是校验谓词缺陷而非设备数据损坏。（3）override 生命周期：夹具新增可选 `preApplyHook`，在 `calculateFlux()` 消耗 override 之后、`apply()` 之前重装；CPU 腿不传 hook，行为逐字节不变（复跑记录与 S0 验收记录逐位全等）。同一波修复了夹具反序列化两个潜伏 bug（`readPoints` 逐组件重复取 token 应为单次 token 冒号拆分；CRLF 记录使 `topLevelSetValid` 严格比较失败）。验收证据：Vulkan 腿在 Release/Intel Arc 上 GREEN——`viennaps-vulkan-surface-process-acceptance-smoke` 两次确定性 exit 0，完整记录（通量、覆盖率、表面点、materialId、processTime、callbackSequence）与配对 CPU 记录逐位全等，五 stage 共享同一 session，负例电池 4/4 fail-closed；聚焦回归 46/46 Vulkan smoke、`hrleSparseReconstruction`、`hrleRebuildCpuFixture`、`levelSetRebuildHandledMatrix`（3 变体）全数通过；N2 中性 oracle 输入按 diff 范围未被触及。`P5-SURFACE-INTEGRATION` 本地转绿；关键路径下一卡 `P5-M0-MODEL-MATRIX-AGGREGATE`。本检查点不 push。

Wave 3（2026-08-20，M0）：`P5-M0-MODEL-MATRIX-AGGREGATE` 在验证互斥锁下完成 15 行支持矩阵聚合收口。只读 gap 分析将四份 artifacts（inventory、compute map、fallback smoke、状态看板）与 `checkInput` 实际谓词逐行核对后，本卡只做文档对齐与两处代码邻近收紧：（1）NeutralTransport 准入谓词补 `getParticleDataLabels().size() == 1U` 校验，与 SingleParticleProcess 行对齐，S1 验收配置满足该条件、证据保持有效；（2）`model_matrix_fallback_smoke.cpp` 与引擎头/源注释如实改写三类准入切片（零反射 SingleParticleProcess、bounded 一反射、NeutralTransport maxReflections<=2，后两者需 frontier SPIR-V 路径非空），消除"只有零反射单跳合格"的过时绝对化表述。broad-switch 审计确认不存在广义 Vulkan 开关：`backendPolicy.hpp` 的 AUTO 仅经逐 stage 硬门槛（base Vulkan suite、strict-FP32 numerical smoke、显存预算、ray mode、FP64 suite），`ComputeBackend::AUTO` 自身永不可选。验收：谓词收紧后 Vulkan 树重建干净，Release/Intel Arc 46/46 smoke 全数通过（含 S1 验收 smoke 位级相等与 fallback smoke 15 行路由断言）；CPU 侧行为按 diff 范围未被触及。`P5-MODEL-MATRIX` 转 `DONE-LOCAL`；下一卡 `P5-K1-TOP-LEVEL` 顶层门禁与发布侧支持文档。本检查点不 push。

Wave 3 续（2026-08-21，K1 准备）：当前提交 `5768825` 已具备 S1 与 M0 前置，最终长套件卡统一记为 `P5-K1-TOP-LEVEL`，以避免同历史 strict-FP32 policy seam 的 `P5-K1` 重名。准备先检查当前源码与 ViennaTools 源码接入：根 CMake 在 `VIENNAPS_BUILD_TESTS=ON` 时纳入 `tests/` 并由 `ViennaPS_Tests` 聚合所有普通测试目录；CPM 使用带 P5-N1 KD-tree 补丁缓存键的 ViennaCore 2.2.1、ViennaLS 5.8.5、ViennaHRLE、ViennaRay 4.3.1 和 ViennaCS。S1 的 HRLE 修复与 ViennaHRLE `isOutsideOfDomain` 的真实 reflective/periodic 语义一致，M0 `checkInput` 的三类窄 Vulkan slice 与源码一致，无广义 Vulkan 开关。K0 的 94-test/`backendPolicy` 单点通过和 `CSVFileProcess` 缺失可执行文件只构成历史诊断，不能替代 K1。准备记录要求在单一验证互斥槽内新建最终候选配置、串行构建 `ViennaPS_Tests`、重新取 CTest inventory、先跑 CPU/S1/M0 focused cluster，最后以单 worker 跑完整非 Benchmark/Performance CTest，并审计子进程。此检查点未运行 configure/build/CTest/Vulkan、未改产品代码、未删除任何生成证据、未宣称 K1 通过。
