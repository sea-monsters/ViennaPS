...
int main(int argc, char **argv) {
  if (argc != 3) {
    std::cerr << "usage: wetetch_process_cpu_checker reference mod\n";
    return 2;
  }
  ...
  if (!reference || !mod || referenceText.str() != modText.str()) {
    std::cerr << "wetetch Process CPU reference differential RED\n";
    return 1;
  }
  std::cout << "wetetch Process CPU reference differential PASS raw_exact\n";
  return 0;
}
