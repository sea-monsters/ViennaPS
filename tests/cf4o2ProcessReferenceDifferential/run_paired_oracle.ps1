...
$source = Join-Path $PSScriptRoot 'cf4o2_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'cf4o2_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_CF4O2_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'cf4o2_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'cf4o2_reference.exe'
$checker = Join-Path $OutputDirectory 'cf4o2_cpu_checker.exe'
