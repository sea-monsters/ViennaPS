...
$source = Join-Path $PSScriptRoot 'oxide_regrowth_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'oxide_regrowth_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_OXIDEREGROWTH_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'oxide_regrowth_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'oxide_regrowth_reference.exe'
$checker = Join-Path $OutputDirectory 'oxide_regrowth_cpu_checker.exe'
