# SF6O2 CPU reference differential

This card records only the rank-5 `SF6O2Etching<float, 2>` CPU/fallback row.
The fixture uses one deterministic `CPU_TRIANGLE` plane, fixed seed `42`, one
ray per point, `maxReflections=0`, and source normalization. It serializes all
five active CPU labels exposed by the model:
...
This is not a Vulkan plasma transport or surface route. Auto remains
