...
2. **Rank 1 now has a locally accepted narrow frontier ray slice**
   (`P5-S1-SURFACE-INTEGRATION-GREEN`) after `P5-N2-NEUTRAL-ORACLE-GREEN` and
   shared surface integration. It remains a compute-only numeric operation, not
   a full `NeutralTransport` model support row.
...
- Existing Mod-side CUDA conversion is intentionally treated as non-Vulkan
  ...
  Vulkan.
- The `P5-NEUTRAL-CPU-ORACLE` Release optimization crash was closed by
  `P5-N2-NEUTRAL-ORACLE-GREEN` (CPM-patched ViennaCore cache headers, OMP 1/2/4/8
  raw-equal). That green oracle unlocked `P5-S1-SURFACE-INTEGRATION-GREEN`, but it
  does not promote the full `NeutralTransport` model row beyond the admitted
  frontier slice.
