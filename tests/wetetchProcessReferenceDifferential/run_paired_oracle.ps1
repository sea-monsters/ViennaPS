...
$source = Join-Path $PSScriptRoot 'wetetch_process_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'wetetch_process_cpu_checker.cpp'
...
function BuildFixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += ' /DVIENNAPS_WETETCH_PROCESS_ORACLE_MOD' }
...
$modExe = Join-Path $OutputDirectory 'wetetch_process_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'wetetch_process_reference.exe'
$checkerExe = Join-Path $OutputDirectory 'wetetch_process_cpu_checker.exe'
