...
$source = Join-Path $PSScriptRoot 'multi_particle_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'multi_particle_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_MULTI_PARTICLE_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'multi_particle_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'multi_particle_reference.exe'
$checker = Join-Path $OutputDirectory 'multi_particle_cpu_checker.exe'
