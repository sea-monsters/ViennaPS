...
- Status: implemented and CPU-executed (historical 2026-08-03); the current
  top-level gate is tracked by `P5-K1-TOP-LEVEL`
- Date: 2026-08-03
...
| Residual top-level gate | the full top-level CTest receipt is now owned by `P5-K1-TOP-LEVEL`; the VTK-enabled install/export variant is separately blocked by the ViennaLS/VTK export-set conflict recorded below |
...
| `PD5-VTK-INSTALL-EXPORT` | `DONE-LOCAL` on 2026-08-21 — supersedes `BLOCKED-LOCAL-ENV`. A standalone VTK 9.6.2 build/install is consumed through find_package imported targets; root USE_VTK=ON configure, full build, install, and the independent installExportConsumer fixture all pass against the two card-local prefixes. In-tree add_subdirectory can never satisfy the ViennaLS export set (architecture note in formal-exit plan §25); sixteen differential-fixture stubs gained VIENNALS_USE_VTK guards so sources compile under both dependency configurations. Consumer runtime requires VTK DLLs plus vcomp140 beside the executable or on PATH. | Owns the optional VTK packaging variant evidence only; no Process preview or hardware promotion implied. | Remote hosted-CI remains the sole external gate for formal P5 closeout. |
...
|---|---|---|---|
| `P5-FORMAL-EXIT-EXECUTION-PLAN` | `RUN` — approved by the user on 2026-08-19 with all three recommended decisions confirmed. It is the governing dependency and dispatch contract for the complete P5 closeout. The execution amendments serialize all local validation and establish a local-only Wave interval plus one reviewed remote push boundary, reducing repeated trusted-execution reviews without bypassing security controls. | Own [the P5 Formal Exit plan](p5-formal-exit-execution-plan.md), its glossary terms, validation mutex, trusted-execution review minimization protocol, and record synchronization only. Implementation ownership remains with the individual cards. | Current P5 closeout work must follow `P5-X0 -> P5-N1/N2 -> P5-S0/S1 -> P5-M0 -> P5-K1-TOP-LEVEL -> P5-E0`. Read-only preparation may be concurrent, but at most one CMake/build/CTest/reference/Vulkan workload may run locally. Sub-agents do not own validation or network access; the main line performs one verified push per accepted Wave. `P5-E0` alone may mark the formal exit complete. |
...
| `P5-M0-MODEL-MATRIX-AGGREGATE` | `DONE-LOCAL` — serialized main-line acceptance on 2026-08-20 after `P5-S1`. | Own the aggregate alignment of the 15-row support matrix across the inventory, compute map, fallback smoke, and status board, plus the NeutralTransport predicate label-count tightening and comment accuracy. No new model semantics and no production routing change. | All four artifacts agree on every row: admitted narrow Vulkan rows are `SingleParticleProcess<float,2>` zero-reflection (Row-01), its bounded one-reflection slice (`P5-RAY-MULTIBOUNCE-PROCESS-INTEGRATION`), the `NeutralTransport<float,2>` frontier slice (`P5-S1`), and the narrow WetEtching/SelectiveEpitaxy/single-precursor TEOS numeric rows; every other family is an explicit CPU-fallback/unsupported row with a named missing-semantics reason and CPU differential evidence. `model_matrix_fallback_smoke` and the S1 acceptance smoke both pass on Release/Intel Arc (46/46 Vulkan smokes); the NeutralTransport admission predicate gained the single-data-label check with S1 evidence intact. Audit confirms no broad Vulkan enable switch: AUTO selection passes only through the strict per-stage hard thresholds in `backendPolicy.hpp`. Unlocks `P5-K1-TOP-LEVEL` and release-facing support documentation; no aggregate production or release claim. |
| `P5-K0-TOP-LEVEL-PREFLIGHT` | `DONE-DIAGNOSTIC / INCOMPLETE-BUILD` after serialized main-line acceptance on 2026-08-19. Its historical receipt did not unlock the final suite; after S1/M0, `P5-K1-TOP-LEVEL` is now `READY-S / PREPARED`, not accepted. | Own only the CPU-only top-level inventory, one focused test boundary, timeout/flaky classification, process audit, and [K0 preflight record](p5-k0-long-suite-preflight.md). It does not own product fixes, Vulkan acceptance, or the integrated long-suite gate. | The pre-mutex `--parallel 2` focused build is historical preparation evidence only. Under the mutex, 94 tests were discovered and `backendPolicy` passed 1/1 in 0.14 s. `CSVFileProcess` was then `Not Run` because its executable was absent, so the remaining 92 tests and all Vulkan coverage were not run. K1 must first build its selected targets under the current serial policy, then execute the integrated suite one test at a time. |
| `P5-K1-TOP-LEVEL` | `DONE-LOCAL` on 2026-08-21 after `P5-K1-R2` adjudicated the single classified RED. The name is deliberately distinct from the older strict-FP32 policy seam called `P5-K1`. | One final-candidate worktree, source-provenance receipt, aggregate CPU test build, inventory, focused cluster, serial non-benchmark CTest, classification, and descendant audit. Product code, dependency edits, deployment, and generated-evidence cleanup are prohibited. | Against the committed `5768825` candidate: patched CPM provenance recorded; serial `ViennaPS_Tests` build exit 0 (60 executables); inventory 95; focused CPU cluster 4/4 and both S1/M0 Vulkan smokes green on Intel Arc after a scoped runtime-DLL wiring fix. The fifteen reference-differential runners were consolidated into one shared engine with an explicit `-BuildDirectory` contract and went 0/15 to 15/15 inside the card. First suite pass: 93/94; the RED (`vulkanCpuBaseline`, uncaught `lineCount > 0`) was adjudicated as a stale expectation — ViennaLS 5.8.5 `ToDiskMesh` is point-cloud-only by contract (reference-identical source) — and fixed test-only, with assertion failures now surfaced readably. Final clean rerun: **94/94 PASS in 700.93 s** plus the Vulkan pair 2/2. See the [execution receipt](p5-k1-final-long-suite-preparation.md). Unlocks the K1 input to `P5-E0`. |
...
| `PD5-CI-REMOTE` | `WAIVED-BY-USER-DECISION` — live GitHub read-only query on 2026-08-18 confirms `master` as the default branch, the remote `master` workflow is the older test-only file, workflow run list is empty, and registered self-hosted runner count is zero. On 2026-08-21 the user deferred the hosted-CI task: the reviewed wave snapshot IS published (`d510614..d1bf5ef` on `codex/p5-closeout-base`, formal-exit §26), but the same-day P5 completion declaration escalated the deferral to a full waiver; run-ID/URL acquisition remains available later through the P7-R4 hosted-lane dependency. No local acceptance row was altered. | Remote branch publication, workflow trigger, and run evidence only. | Publish a branch containing the current workflow; capture commit SHA and hosted CPU/no-SDK run IDs/URLs, then unlock release-facing install/export evidence. A separately provisioned `self-hosted,vulkan` runner is required for the optional hardware lane. |
...
| `P5-SURFACE-INTEGRATION` | `S1-GREEN DONE-LOCAL` — the deterministic S0 RED boundary is historical and was closed by S1 on 2026-08-20; the paired Release NeutralTransport CPU oracle is green under `P5-N2`. | Coverage, surface diffusion, neutral velocity, and complete `Process::calculateFlux/apply` callback installation. | The S1 surface fixture passes twice on Release/Intel Arc with bit-exact flux, coverages, surface points, material ids, process time, and callback sequence against the paired CPU record; the negative battery remains 4/4 fail-closed. No production-route promotion or full-model claim is implied. |
| `P5-MODEL-MATRIX` | `DONE-LOCAL` — aggregate 15-row matrix closed under `P5-M0` on 2026-08-20 after `P5-N2-NEUTRAL-ORACLE-GREEN` and `P5-S1-SURFACE-INTEGRATION-GREEN`; Row-01, the bounded SingleParticle one-reflection slice, the `NeutralTransport<float,2>` frontier ray slice, the MultiParticle, IonBeam, CF4O2, SF6O2/SF6C4F8, concrete Fluorocarbon, SingleParticleALD, TEOSPECVD and OxideRegrowth CPU/fallback rows, WetEtch, SelectiveEpitaxy and single-precursor TEOS are narrow rows; aggregate production promotion remains locked. | Multi-particle/species Vulkan transport, full NeutralTransport model semantics (coverage/desorption/diffusion/material sticking), ion/neutral material semantics, fluorocarbon/plasma/TEOS, wet-etch/selective-epitaxy/oxide-regrowth coverage. | Each additional row needs its own transport/surface seam, paired CPU/reference oracle, conservation/geometry evidence, and explicit fallback; aggregate promotion remains locked. |
| `P5-X1-HYGIENE-CLEANUP` | `EXECUTED-TIER-A` on 2026-08-21 — the P5 completion declaration triggered the bound wave. Removed the four verified-clean worktrees (branch refs preserved, commits reachable via their branches), stray `vc140.pdb`, and ~11.8 GB of `.tmp_p5_*` evidence trees (K1/E0/N/S waves) whose receipts live in versioned records; retained by name: both install prefixes (`vtk_install`, `viennaps_install`) and the instrumented `.tmp_reference_viennaray` test asset. | Classify and remove only CLEAN-SAFE targets: four clean secondary worktree candidates, the `.tmp_p5_*` validation-evidence wave, and stray root artifacts (`vc140.pdb`, `MakePlane_Substrate`, `coverage_metrics.txt`) after ownership verification. | Tier-B residual: ten dirty worktrees are RETAINED pending ownership classification of their untracked content; no deletion occurred without a clean status plus branch-reachability audit. Prohibited without classification: deleting user-owned untracked content (`.claude/`, any file without an ownership record), dirty worktrees, or dependency caches; every deletion needs a process audit plus an evidence note. Predecessor: hosted-CI evidence completing formal P5 closeout. |
| `P5-E0-FINAL-AUDIT` | `DONE` on 2026-08-21 by user completion declaration; the hosted-CI evidence gate was explicitly waived in the same decision (see `P5-DEPLOYMENT-EXIT`). All locally executable acceptance rows had already passed. The user-facing Preview boundary artifact is `DEFERRED-WITH-DEPLOYMENT` by decision 2026-08-21: it has no audience before deployment exit and will be produced with that gate instead of as a placeholder. The VTK-enabled install/export variant closed locally on 2026-08-21 via a standalone VTK 9.6.2 install consumed through find_package (see formal-exit plan §25). | Self-audit the complete diff, rerun focused CPU/Vulkan gates, adversarially validate the whole P5 Vulkan surface, root-cause any finding, and update records. Remote publication is a separate main-line action. | Pass A: CPU tree 94/94 (562.62 s) and Vulkan-enabled tree 132/132 (558.60 s) serial non-benchmark suites on the fixed runtime. Pass B: all 30 `viennaps-vulkan-*` smokes under forced `VK_LAYER_KHRONOS_validation` report zero VUID diagnostics after two root-cause fixes — the ray-reducer push-constant layout now mirrors its shader block (`VUID-VkComputePipelineCreateInfo-layout-07987`), and a process-wide resource ledger frees abandoned allocations/handles during device teardown without breaking stale-handle rejection (`VUID-vkDestroyDevice-device-05137`). One latent exact-equality flake across TBB-scheduled CPU reductions was re-contracted to a tight relative tolerance. Pass C: static audit found no unchecked VkResult failures beyond two void fence-rearm calls whose failure is visible downstream, no swallow-catches, no broad backend switch, and no machine-local paths. See formal-exit plan §24. |
| `P5-DEPLOYMENT-EXIT` | `DONE-BY-USER-DECLARATION` on 2026-08-21 — the user directly declared P5 complete and waived the hosted-CI evidence gate (`PD5-CI-REMOTE` waived; snapshot already published per formal-exit §26). `P5-MODEL-MATRIX` and `P5-K1-TOP-LEVEL` are DONE-LOCAL and every locally executable acceptance row was green before declaration. Recorded as explicit authority decision, not silent conversion. | Install/export variants, deployment profile, Process preview documentation, and support matrix. The local opt-in Vulkan payload is now present, but no automatic Process promotion is implied. | Hosted install/export plus optional VTK-enabled case, deployment-profile persistence, release-scoped Vulkan Process preview, and final support matrix; unlocks P6/P7 integration. |
| `P6-LA-BASELINE` | `DONE-LOCAL` on 2026-08-22 — the deterministic `viennaps::la` CPU layer (CSR/Jacobi/BiCGSTAB) is production, with bit-identical iterate sequences across OMP 1/2/4/8 (solution fingerprint `0x6ab01e286ce287d5`, history `8c6a980f16da9004`). | Own the CPU linear-algebra authority for every P6 device claim; pure library headers under `include/viennaps/la`, no Process/Vulkan types. | Four-facet fixture green (hand-checked assembly, frozen-SPD cross-thread equality, typed failures, spmv dense reference); `/W4 /WX` clean. |
| `P6-OXIDATION-COUPLING` | `DONE-LOCAL` on 2026-08-22 after premise-corrected rescoping. | A2: opt-in stage telemetry + parity receipts on the EXISTING orchestrator (the 2026-08-05 decompose-the-monolith premise was obsolete - psOxidation already stages internally). A3: completion contract eliminating silent success on stalled coupling loops. | Telemetry-on/off runs match structurally and geometrically (tight tolerance); done-record verdict asserted; ViennaLS rollback evidence documented (retry/rejection precedes advection). Commits 719c1b8, 9272a59. |
| `P6-A4-FP64-KERNELS` | `DONE-LOCAL (DEVICE-PENDING-HARDWARE)` on 2026-08-22. | FP64 SpMV/AXPY/dot shaders behind VIENNAPS_ENABLE_VULKAN_FP64 (default OFF); static SPIR-V validation is the local receipt; execution requires an FP64-capable device. | Three kernels validate clean; fp64_smoke proves policy fail-closed pre-submit (AUTO->CPU, MANUAL names FP64); flag-OFF regression clean. Commit 3bc15a8. |
| `P6-PHYSICS-EXIT` | `DONE-LOCAL` on 2026-08-22 via stage-gating matrix + dual long suites. | Aggregate stage support matrix (CPU-authoritative rows + DEVICE-PENDING rows), fallback verification, K1-style top-level receipts. | stageGatingMatrix five-row matrix green in BOTH trees; CPU long suite **96/96**; Vulkan tree **135/135 in 543.06 s**. Wave absorbed two structural fixes: per-executable runtime-DLL self-provisioning and git-common-dir cache resolution for linked worktrees. ViennaCore cache-key drift checked benign (both keys carry N1 volatile fix). See p6-p7-execution-plan.md §6. | FP64 matrix assembly, SpMV, AXPY/dot/norm, deterministic reduction, BiCGSTAB/Jacobi, OOM admission. | 2D/3D field and matrix CPU/Vulkan differential with convergence/residual history; unlocks oxidation stages. |
| `P7-RESIDENT-EXECUTION` | `READY-S` after P5 and P6 exits.
| `P7-R0-BASELINE-FREEZE` | `DONE-LOCAL` on 2026-08-22 - branch `codex/p7-base` from accepted P6-EXIT (59cb9cf); 7.0 GB of P6 wave build trees swept. |
| `P7-R1-RESIDENT-WORKING-SET` | `DONE-LOCAL` on 2026-08-22 (a4e5aa0). | ResidentWorkingSet: generation-scoped liveness over RAII-owned buffers, per-entry invalidation counters, auto-invalidation on dead generations; E0 ledger guarantee preserved. | Real-hardware receipts on Intel Arc: bit-identical routes, resident transfers 2 MiB vs 8 MiB P5-style (4x reduction), green under forced Khronos validation. |
| `P7-R2-SELECTION-RECORDS` | `DONE-LOCAL` on 2026-08-22 (d863f90). | Deterministic schema-versioned records over buildSelectionPlan: profile digest, workload descriptors, per-stage decisions + reasons; pure function of inputs. | Byte-equal determinism/replay verified; mutation flips replay; local-Arc FP64 rejection isolated inside the record via passing strict-FP32 evidence shape. |
| `P7-R3-CALIBRATION` | `DONE-LOCAL` on 2026-08-22 (553bfe6). | Schema-versioned CalibrationRecord with deterministic serialization; measured CPU workload (deterministic la-layer SpMV, median of 7). | Finite positive median receipt; byte-deterministic serialization; FP64 entries await hosted lane. |
| `P7-R4-CI-SOAK-RELEASE` | `LOCAL-DELIVERABLES-DONE / HOSTED-SOAK BLOCKED-EXTERNAL (DEFERRED-BY-STANDING-DECISION)` on 2026-08-22. | Release support matrix published (p6-p7-execution-plan.md §4); six-fixture soak-substitute batch green (4 CPU + 2 device incl. validation layer). | Hosted runner provisioning remains the external dependency for real soak runs and device-FP64 evidence; recorded as standing decision, never a silent conversion. | | Cross-step device working set, callback invalidation/detach telemetry, command segmentation, and cache lifetime. | Repeated multi-step process run has bounded transfers and correct invalidation/recovery; unlocks calibration. |
...
`P5-K1-TOP-LEVEL` -> `P5-E0-FINAL-AUDIT`. `P5-K0` and local deployment preparation may run in
...
### P5 gap-closure execution board (historical snapshot: 2026-08-06; superseded)

The rows below preserve the original diagnostic and planning evidence. The
current card states are the `P5 to P7` continuation board above; later N1H/N2,
S1, and M0 acceptance supersede any older `BLOCKED` or `EVIDENCE-GAP` wording
inside this historical table.
...
| `P5-NEUTRAL-CPU-ORACLE` | `DONE-LOCAL` — superseding the historical `EVIDENCE-GAP-MAIN` note; the N1H patch adoption plus the N2 main-line gate re-ran the full acceptance matrix on 2026-08-20: Mod and unmodified reference, OMP 1/2/4/8, exact Release flags, identical dependency closure, exit 0 with raw serialized equality (`max_ulp=0`) at every thread count, and the default-flags differential unchanged. See formal-exit plan §17–§18. | Closed the Release Neutral CPU oracle milestone that gated surface integration. | Unlocked `P5-S0` and the velocity-substage evidence path. |
...
| `P5-SURFACE-INTEGRATION-IMPLEMENTATION` | `DONE-LOCAL` — superseding the historical `BLOCKED-ORACLE` note; the oracle blocker was removed by `P5-N2`, after which `P5-S0` froze the complete surface fixture (RED at exactly the predicted device-physics boundary) and `P5-S1` closed it GREEN with bit-exact paired CPU/Vulkan records on Release Intel Arc. See formal-exit plan §19–§20. | Owned only the three adapters the S0 RED boundary required plus fixture deserializer fixes; CPU formulas and order untouched. | Unlocked `P5-M0`; no production-route promotion beyond the admitted slices. |
...
| `P5-MODEL-MATRIX-ROWS` | `DONE-LOCAL` — all 15 rows carry an exact predicate, CPU differential, fallback behavior, and unsupported reason as of `P5-M0` (2026-08-20), following `P5-N2-NEUTRAL-ORACLE-GREEN` and `P5-S1-SURFACE-INTEGRATION-GREEN`; Row-01 plus the bounded SingleParticle one-reflection slice and the `NeutralTransport<float,2>` frontier ray slice, plus CPU/fallback evidence for MultiParticle, IonBeam, CF4O2, SF6O2, SF6C4F8, concrete Fluorocarbon, SingleParticleALD, TEOSPECVD and OxideRegrowth still do not open the aggregate lane. | Implement one model row at a time from `docs/design/p5-model-matrix-inventory.md`; Vulkan eligibility is opt-in per evidenced predicate, all other rows remain CPU fallback/unsupported. Do not add a global Vulkan switch or infer CUDA support. Rank-14 Oxidation is explicitly deferred to the P6 managed-solver phase. | Each row must provide a reference-CPU differential, conservation/geometry checks appropriate to its semantics, explicit fallback or fail-closed behavior, and a named missing-semantics reason. Stop and reclaim after one correction failure. | `P5-DEPLOYMENT-EXIT` only after every claimed P5 row and the CPU surface blocker are resolved. |
| `P5-MODEL-MATRIX-COMPUTE-MAP` | `DONE-LOCAL` after the 15-row inventory and the S1 NeutralTransport frontier update; it is design evidence, not a support update. | Own [p5-model-matrix-compute-map.md](p5-model-matrix-compute-map.md) only. Map the CPU R/C/D/L/M computation surface, missing device kernels, retained CPU semantics, and row-specific oracle for strict plus all 14 non-eligible rows. | The map rejects CUDA classes, generic callbacks, and fallback configuration as Vulkan support. Neutral velocity now has a locally accepted narrow frontier ray slice (`P5-S1`) after `P5-N2`; the full `NeutralTransport` model row remains CPU fallback/Manual fail-closed. WetEtching, SelectiveEpitaxy, and single-precursor TEOS retain their narrow numeric/production Process evidence, and every other complete non-strict model row remains CPU fallback/Manual fail-closed. | `P5-NEUTRAL-VELOCITY-SUBSTAGE`, the separately accepted WetEtching, SelectiveEpitaxy, and TEOS adapters, without unlocking aggregate matrix. |
...
| `P5-NEUTRAL-VELOCITY-SUBSTAGE` | `DONE-LOCAL-NARROW` after `P5-N2-NEUTRAL-ORACLE-GREEN` and `P5-S1-SURFACE-INTEGRATION-GREEN`; it is still not a full `NeutralTransport` support row. | Own the existing backend-neutral `NeutralTransportVelocityExecutor` compute seam and its focused CPU/Vulkan differential. Preserve the CPU particle transport, coverage, desorption, surface diffusion, material behavior and Process order. | Accept raw-bit velocity candidate equality on the bounded N2 input domain, transactional callback publication, CPU fallback/Manual fail-closed, and a paired full CPU process/conservation/geometry oracle. `P5-S1` adds a `NeutralTransport<float,2>` frontier ray slice with bit-exact paired CPU record on Release/Intel Arc, five-stage shared session, and negative battery 4/4; broader configurations remain unsupported. | Reusable numeric-operation evidence only; aggregate model matrix remains locked. |
...
| `P5-MODEL-MATRIX` | `DONE-LOCAL` after `P5-M0` aggregate acceptance on 2026-08-20: inventory, compute map, fallback smoke, and this board agree on all 15 rows. | One support row at a time; no broad "Vulkan enabled" switch. | Each row has an eligibility rule, CPU differential, conservation/geometry criterion, fallback behavior, and unsupported reason. | `P5-DEPLOYMENT-EXIT`. |
...
| Optional VTK export | `DONE-LOCAL` on 2026-08-21 via standalone VTK 9.6.2 find_package consumption; see formal-exit plan §25 for the full chain (configure, build, install, independent consumer PASS) and the in-tree add_subdirectory architecture finding. Supersedes the 2026-08-18 `BLOCKED-LOCAL-ENV` HDF5 probe note. |
