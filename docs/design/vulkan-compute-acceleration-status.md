...
| `P5-SURFACE-INTEGRATION` | `ORACLE-UNBLOCKED / READY-S` after `P5-S0` — strict callback/configuration and ray rows are locally narrow-accepted, and the former oracle blocker is resolved: the paired Release NeutralTransport CPU oracle is green since 2026-08-20 (`P5-N2`, CPM-patched ViennaCore headers, OMP 1/2/4/8 raw-equal). | Coverage, surface diffusion, neutral velocity, and complete `Process::calculateFlux/apply` callback installation. | The complete surface fixture enters RED under `P5-S0`; no setup or executor smoke substitutes for the paired oracle, which now passes at `/O2 /Ob2 /DNDEBUG /openmp:llvm /MD /Zi`. |
