...
int main(int argc, char **argv) {
  if (argc != 3) {
    std::cerr << "usage: fluorocarbon_cpu_checker reference mod\n";
    return 2;
  }
  ...
  if (referenceBytes != modBytes) {
    std::cerr << "Fluorocarbon CPU reference differential mismatch\n";
    return 1;
  }

  std::cout << "Fluorocarbon CPU reference differential PASS raw_bytes="
            << referenceBytes.size()
