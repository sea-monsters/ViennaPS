// Independent CPU Process oracle for the narrow SelectiveEpitaxy row.  The
// same source is compiled once against the Mod headers and once against the
// unmodified D:\Codex_lib\code_reference\ViennaPS tree.  No Vulkan executor
// is installed; CPU Process/LevelSet semantics remain the oracle.
...
void write(const std::string &path) {
  ...
  viennaps::MakePlane<T, D>(domain, 0.0F, viennaps::Material::Si).apply();
  domain->duplicateTopLevelSet(viennaps::Material::SiGe);
  auto model = viennacore::SmartPointer<viennaps::SelectiveEpitaxy<T, D>>::New(
      std::vector<std::pair<viennaps::Material, T>>{
          {viennaps::Material::Si, 1.0F},
          {viennaps::Material::SiGe, 0.75F}},
      0.5F, 1.0F);
  viennaps::Process<T, D> process(domain, model, 0.02F);
  process.apply();
#ifdef VIENNAPS_SELECTIVE_EPITAXY_PROCESS_ORACLE_MOD
  if (process.getLastProcessResult() != viennaps::ProcessResult::SUCCESS)
    throw std::runtime_error("SelectiveEpitaxy Process did not succeed");
#endif
  ...
  if (!output)
    throw std::runtime_error(
        "cannot open SelectiveEpitaxy Process oracle output");
  output << "schema=1\nprecision=float\ndimension=2\ncount=" << values.size()
...
int main(int argc, char **argv) {
  if (argc != 2) {
    std::cerr << "usage: selective_epitaxy_process_cpu_fixture output\n";
    return 2;
  }
  try {
    write(argv[1]);
