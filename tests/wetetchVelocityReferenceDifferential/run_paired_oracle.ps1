...
$source = Join-Path $PSScriptRoot 'wetetch_velocity_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'wetetch_velocity_cpu_checker.cpp'
...
function BuildFixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += ' /DVIENNAPS_WETETCH_ORACLE_MOD' }
...
$modExe = Join-Path $OutputDirectory 'wetetch_velocity_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'wetetch_velocity_reference.exe'
$checkerExe = Join-Path $OutputDirectory 'wetetch_velocity_cpu_checker.exe'
