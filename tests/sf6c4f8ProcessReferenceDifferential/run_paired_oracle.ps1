...
$source = Join-Path $PSScriptRoot 'sf6c4f8_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'sf6c4f8_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_SF6C4F8_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'sf6c4f8_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'sf6c4f8_reference.exe'
$checker = Join-Path $OutputDirectory 'sf6c4f8_cpu_checker.exe'
