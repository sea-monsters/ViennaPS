...
$source = Join-Path $PSScriptRoot 'teospecvd_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'teospecvd_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_TEOSPECVD_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'teospecvd_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'teospecvd_reference.exe'
$checker = Join-Path $OutputDirectory 'teospecvd_cpu_checker.exe'
