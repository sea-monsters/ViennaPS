...
$source = Join-Path $PSScriptRoot 'teos_process_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'teos_process_cpu_checker.cpp'
...
function BuildFixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  $cmd = 'call ' + (Quote $vsDevCmd) + ' -arch=x64 >nul && cl.exe /nologo /std:c++20 /EHsc /O2 /Ob2 /DNDEBUG /openmp:llvm /MD ' +
    $commonDefines + ' ' + (($includes | ForEach-Object { '/I' + (Quote $_) }) -join ' ') + ' ' +
...
$modExe = Join-Path $OutputDirectory 'teos_process_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'teos_process_reference.exe'
$checkerExe = Join-Path $OutputDirectory 'teos_process_cpu_checker.exe'
