// CPU-authority fixture for the rank-6 SF6C4F8Etching fallback row.
// This is a four-label CPU differential only. It does not admit a Vulkan
// plasma/polymer transport or surface route and does not widen eligibility.
#include <array>
#include <bit>
#include <cmath>
#include <cstdint>
...
void run(const std::string &outputPath) {
  ...
  modelParameters.passivationFlux = T(0);
  modelParameters.beta_E.set(Material::Si, T(.7));
  modelParameters.beta_E.set(Material::Mask, T(.02));
  modelParameters.beta_E.set(Material::Polymer, T(.02));
  ...
  auto model = SmartPointer<SF6C4F8Etching<T, D>>::New(modelParameters);
  ...
  for (std::size_t i = 1; i < values.size(); ++i) {
    if (values[i]->size() != values[0]->size())
      throw std::runtime_error("SF6C4F8 CPU label sizes differ");
  }
  ...
  out << "schema=1\nsource=cpu\nmodel=SF6C4F8Etching\n"
         ...
         "ionEnhancedPassivationFlux,etchantFlux\n";
  for (std::size_t i = 0; i < kLabels.size(); ++i)
...
int main(int argc, char **argv) {
  if (argc != 2) {
    std::cerr << "usage: sf6c4f8_cpu_fixture output\n";
    return 2;
  }
  try {
    run(argv[1]);
