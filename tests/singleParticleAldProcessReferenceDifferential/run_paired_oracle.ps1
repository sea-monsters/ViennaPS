...
$source = Join-Path $PSScriptRoot 'single_particle_ald_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'single_particle_ald_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_SINGLE_PARTICLE_ALD_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'single_particle_ald_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'single_particle_ald_reference.exe'
$checker = Join-Path $OutputDirectory 'single_particle_ald_cpu_checker.exe'
