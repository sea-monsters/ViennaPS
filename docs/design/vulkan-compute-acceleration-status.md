...
|---|---|---|---|
| `P5-FORMAL-EXIT-EXECUTION-PLAN` | `RUN` — approved by the user on 2026-08-19 with all three recommended decisions confirmed. It is the governing dependency and dispatch contract for the complete P5 closeout. The same-day execution amendment serializes all local validation after concurrent builds exhausted the runner. | Own [the P5 Formal Exit plan](p5-formal-exit-execution-plan.md), its glossary terms, validation mutex, and record synchronization only. Implementation ownership remains with the individual cards. | Current P5 closeout work must follow `P5-X0 -> P5-N1/N2 -> P5-S0/S1 -> P5-M0 -> P5-E0`. Read-only preparation may be concurrent, but at most one CMake/build/CTest/reference/Vulkan workload may run locally. `P5-E0` alone may mark the formal exit complete. |
...
| `P5-N1-NEUTRAL-ROOTCAUSE-DISCRIMINATION` | `RECLAIMED-MAIN / BLOCKED-EXTERNAL` after serialized main-line acceptance on 2026-08-19; `P5-N2` is not unlocked. | Own only the phased paired runner, Neutral oracle record, isolated compiler/dependency evidence, and the disabled test-only explicit `KDTree<float, array<float,3>>` specialization candidate. CPU/KDTree formulas, Process/model semantics, reference sources, required Release/OpenMP flags, Vulkan eligibility, and support rows remain prohibited. | Hostx86/x64 and Hostx64/x64 Mod Release builds completed under `/O2 /Ob2 /DNDEBUG /openmp:llvm /MD /Zi`. The correctly configured Hostx86 Mod OMP=1 lane stopped making CPU progress during coverage initialization. The phased Hostx64 Mod baseline and the one authorized explicit-KDTree code-generation candidate each timed out after 180 seconds with zero-byte oracle output; all exact processes were reaped. The initial Hostx86 `VIENNAPS_NEUTRAL_ORACLE_MOD=OFF` run is explicitly invalid evidence. Because Mod OMP=1 never passed, reference, raw comparison, and OMP 2/4/8 were not run. The candidate is rejected and no second local candidate is authorized. | An upstream reproducer/fix, a supported compiler/runtime closure, or a separately approved repair boundary; until then `P5-N2`, complete surface integration, aggregate matrix, and formal exit remain locked. |
...
| `P5-SURFACE-INTEGRATION` | `BLOCKED-ORACLE` — strict callback/configuration and ray rows are locally narrow-accepted, but the required Release paired NeutralTransport CPU oracle still fails in the shared KDTree path. | Coverage, surface diffusion, neutral velocity, and complete `Process::calculateFlux/apply` callback installation. | The full surface fixture cannot enter RED/GREEN until the paired `/O2 /Ob2 /DNDEBUG /openmp:llvm /MD /Zi` CPU oracle passes; no setup or executor smoke substitutes for it. |
