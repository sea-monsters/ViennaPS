...
$source = Join-Path $PSScriptRoot 'ion_beam_cpu_fixture.cpp'
$checkerSource = Join-Path $PSScriptRoot 'ion_beam_cpu_checker.cpp'
...
function Build-Fixture([string]$Output, [string]$ViennaPS, [bool]$Mod) {
  ...
  if ($Mod) { $defines += '/DVIENNAPS_ION_BEAM_MOD' }
  $includeArgs = @(
...
$modExe = Join-Path $OutputDirectory 'ion_beam_mod.exe'
$referenceExe = Join-Path $OutputDirectory 'ion_beam_reference.exe'
$checker = Join-Path $OutputDirectory 'ion_beam_cpu_checker.exe'
